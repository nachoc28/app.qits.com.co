            ...
            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-200 px-6 py-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        @if(!empty($canDownloadPdf))
                            <a
                                href="{{ route('public.form-notification.pdf', ['token' => $token]) }}"
                                class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                            >
                                Descargar PDF
                            </a>
                        @else
                            <div class="text-right">
                                <button
                                    type="button"
                                    class="inline-flex cursor-not-allowed items-center justify-center rounded-md border border-gray-200 bg-gray-100 px-4 py-2 text-sm font-medium text-gray-400"
                                    disabled
                                    aria-disabled="true"
                                >
                                    Descargar PDF
                                </button>
                                <p class="mt-1 text-xs text-gray-500">Descarga PDF temporalmente no disponible.</p>
                            </div>
                        @endif
                    </div>
                </div>

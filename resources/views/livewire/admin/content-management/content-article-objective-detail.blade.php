<div class="space-y-6">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Esta vista resume el articulo y opera los pasos actualmente habilitados del flujo de contenidos.
                </p>
            </div>
            ...
            <div class="flex flex-wrap gap-2 text-xs">
                ...
                <span class="inline-flex rounded-full border border-gray-200 bg-gray-100 px-3 py-1 font-semibold text-gray-800">
                    Paso objective: {{ optional($objectiveStep)->step_status ?? 'missing' }}
                </span>
            </div>
        </div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Objetivo refinado</p>
                <p class="mt-1 text-sm text-gray-900 whitespace-pre-line">{{ $article->refined_objective ?: '-' }}</p>
            </div>

            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Publico objetivo refinado</p>
                <p class="mt-1 text-sm text-gray-900 whitespace-pre-line">{{ $article->refined_target_audience ?: '-' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.95fr)]">
        <div class="space-y-6">
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 class="text-base font-semibold text-gray-900">Generacion de Prompt 1</h3>
                        <p class="mt-1 text-sm text-gray-500">
                            Cada generacion guarda un snapshot exacto del prompt final y de la version activa de plantilla.
                        </p>
                    </div>
                ...
                <div class="mt-5">
                    <label for="content_objective_prompt_preview" class="text-sm font-medium text-gray-700">Prompt seleccionado</label>
                    <textarea
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                <div>
                    <h3 class="text-base font-semibold text-gray-900">Resultados refinados manuales</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        Estos campos se capturan manualmente y no alteran el objetivo estrategico general ni el publico objetivo general importados.
                    </p>
                </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                <h3 class="text-base font-semibold text-gray-900">Historial de generaciones</h3>
                <p class="mt-1 text-sm text-gray-500">
                    Cada clic en generar o regenerar crea una fila nueva en el historial.
                </p>
                ...
                <div class="mt-5 space-y-3">
                        ...
                        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div class="min-w-0">
                                    ...
                                    <div class="mt-1 text-xs text-gray-600">
                                        Plantilla: {{ optional(optional($generation->templateVersion)->masterTemplate)->key ?: 'objective' }}
                    ...
                    @empty
                        <div class="rounded-xl border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-sm text-gray-500">
                            Aun no hay generaciones para el paso objective.
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            @if($objectiveStep)
                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
                    <h3 class="text-base font-semibold text-gray-900">Estado del paso objective</h3>

                    <dl class="mt-5 space-y-3 text-sm text-gray-700">
                        <div class="flex items-start justify-between gap-4">
                            <dt class="font-medium text-gray-600">Estado</dt>
                            <dd class="text-right">{{ $objectiveStep->step_status }}</dd>
                        </div>
                        <div class="flex items-start justify-between gap-4">
